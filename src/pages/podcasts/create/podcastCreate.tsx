...
import { SingleFileCover } from "@/components/custom/SingleFileCover"
import Breadcrumbs from "@/components/custom/breadcrumbs"
import { Categories } from "@/pages/categories/shema"
import { Authors } from "@/pages/authors/shema"
import MultiSelectCustom, { MultiSeclectOptions } from "@/components/custom/MultiSelectCustom"
...
export default function PodcastCreate() {
  ...
  const { data: CategoriesOptions = [], isLoading } = useQuery<MultiSeclectOptions[] | undefined>({
    ...
    queryFn: async () => {
      ...
      return res.data.data.map(item => ({ value: item.id, label: `${item.name?.en}-${item.name?.vi}` })) ?? []
    }
  })
